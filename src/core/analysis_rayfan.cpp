...
void
RayFan::set_plane (enum rayfan_plane_e plane)
{
...
double
RayFan::get_entrance_height (const trace::Ray &r,
                             const trace::Ray &chief) const
{
  ...
  return ray->origin ()[_dist_plane]
         / _entrance->get_shape ().get_outter_radius (
             math::Vector2 (1 - _dist_plane, _dist_plane));
}

double
RayFan::get_entrance_angle (const trace::Ray &r, const trace::Ray &chief) const
{
  const trace::Ray *ray = &r;
  ...
  if (ray && (ray = ray->get_parent ()))
    return math::rad2degree (
        atan (ray->direction ()[_dist_plane] / ray->direction ().z ()));

  throw Error ();
}

double
RayFan::get_transverse_distance (const trace::Ray &r,
                                 const trace::Ray &chief) const
{
...
double
RayFan::get_longitudinal_distance (const trace::Ray &r,
                                   const trace::Ray &chief) const
{
  if (&r == &chief)
    throw Error ();
...
double
RayFan::get_exit_angle (const trace::Ray &r, const trace::Ray &chief) const
{
  ...
  return math::rad2degree (
      atan (ray->direction ()[_ab_plane] / ray->direction ().z ()));
}

double
RayFan::get_image_angle (const trace::Ray &r, const trace::Ray &chief) const
{
  return math::rad2degree (
      atan (r.direction ()[_ab_plane] / r.direction ().z ()));
}

double
RayFan::get_optical_path_len (const trace::Ray &r,
                              const trace::Ray &chief) const
{
...
const trace::Ray &
RayFan::find_chief_ray (const trace::rays_queue_t &intercepts, double wavelen)
{
  for (auto &i : intercepts)
    {
      trace::Ray &ray = *i;

      if (ray.get_wavelen () == wavelen
          && fabs (get_entrance_height (ray, /* dummy */ ray)) < 1e-8)
        return ray;
    }
...
RayFan::get_plot (enum rayfan_plot_type_e x, enum rayfan_plot_type_e y)
{
  ...
  for (auto &w : result.get_ray_wavelen_set ())
    {
      ...
      std::shared_ptr<data::DiscreteSet> s
          = std::make_shared<data::DiscreteSet> ();
  ...
  static const struct
  {
    ...
    bool prefix;
    int pow10;
  } axis[] = { { "Entrance ray height (normalized)", "", false },
  ...
  plot->set_title (
      axis[y].label + std::string (" fan (")
      + (_dist_plane == SagittalAberration ? "sagittal)" : "tangential)"));
